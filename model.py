...
def model_fn_meta_SR(features, labels, mode, params):
    ...
    res = res.stack()
    res = tf.transpose(res, perm=[1, 0, 2, 3, 4])
    """当前的多尺度和优化版本训练仅支持batch_size = 1的情况"""
    res_shape = tf.shape(res)
    res = tf.reshape(res, [-1, meta_sr_kernel_size, meta_sr_kernel_size, res_shape[-1]])
    ...
    batch_size = 200000
    def cond2(i, n, array):
        return tf.less(i * batch_size, n)
    def body2(i, n, array):
        value = batch_conv(res[i * batch_size: (i+1)*batch_size], output[i * batch_size: (i+1)*batch_size])
        array = array.write(i, value)
        return i+1, n, array

    _, _, array = tf.while_loop(cond2, body2, loop_vars=[0, new_reshape[0], array_2])
    array = array.concat()
